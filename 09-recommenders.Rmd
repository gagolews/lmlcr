...
benchmark sets for recommender systems, which helped boost the research on algorithms in this field.
...
> Will Sara enjoy her spinach?
> Will Sara enjoy her apple?
...
to store ratings data. We don't want to run out of memory!
...
S_C(\mathbf{a},\mathbf{b}) = \frac{\sum_{i=1}^m a_i b_i}{
    \sqrt{\sum_{i=1}^m a_i^2}\sqrt{\sum_{i=1}^m b_i^2}
}
\]
...
the complete dataset that Netflix has):
