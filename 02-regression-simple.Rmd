...
- Recall that an **algorithm** is a well-defined sequence of instructions that,
...
or *an element of a reap $p$-space* etc.).
...
### Prediction vs. Description (Inference)
...
- (\*\*) statistically speaking, this has a clear underlying interpretation
    ...
    > (which we will study
    in our Machine Learning unit = assuming errors are normally distributed,
    finding a model minimising the SSR is equivalent to maximum likelihood estimation)
...
(\*\*) Other choices (not studied in this unit == it's too early for this):

- regularised SSR (for regression with multiple input variables)
...
Pearson's r introduced in the next lecture though
...
(\*\*) In the next lecture, we will introduce the notion of Pearson's
...
as well as
* logical operators like `&` (and) as well as `|` (or)
...
In the next lecture, we will learn about the Pearson linear
