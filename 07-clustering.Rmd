...
In **anomaly detection**, our task is to identify rare, suspicious, ab-normal
...
```{r,echo=FALSE}
par(mar=c(2,2,0.5,0.5))
plot(iris[,3], iris[,2], las=1, ann=FALSE, col=as.numeric(iris[,5]), pch=as.numeric(iris[,5]))
```
...
However, even one of the most famous ML textbooks provides us with only a vague hint:
...
plot(X, col=km$cluster, pch=as.numeric(iris$Species), las=1)
```
...
: Note that the true labels were not used during the clustering procedure!
...
```{r complete_linkage_hier5_intro,fig.height=5}
par(mfrow=c(2,2))
...
par(mfrow=c(1,3))
# single
plot(X[1,], X[2,], col=y, pch='.', asp=1, axes=FALSE)
box()
...
```{r complete_linkage_hier5,fig.height=5}
X <- as.matrix(iris[,c(3,2)])
D <- dist(X)
...
```{r complete_linkage_hier,fig.height=5}
plot(h)
```
...
```{r average_linkage_hier5,fig.height=5}
...
```{r average_linkage_hier,fig.height=5}
plot(h)
```
...
```{r single_linkage_hier5,fig.height=5}
...
```{r single_linkage_hier,fig.height=5}
plot(h)
```
