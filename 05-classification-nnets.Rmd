...
-\frac{1}{n} \sum_{i=1}^n
\Bigg(y_i\log \hat{y}_i + (1-y_i)\log \hat{y}_i\Bigg).
\]
...
```{r mnist_demo,cache=TRUE,echo=FALSE,fig.height=6,fig.width=12}
library("keras")
mnist <- dataset_mnist()
par(mar=c(0,0,0,0))
set.seed(123)
...
```{r mnist_info2b,cache=TRUE,fig.height=5,echo=-1}
par(mar=c(0,0,0,0))
...
```{r cross_entropy_revisited_example3,echo=FALSE}
...
the Creative Commons Attribution-Share Alike 3.0 Unported license](figures/neuron){#fig:neuron width='50%'}
...
![](figures/perceptron){#fig:perceptron width='50%'}
...
We determine a linear combination (a weighted sum) of $p$ inputs
and then transform it using the logistic sigmoid "activation" function.


![](figures/logistic_regression_binary){#fig:logistic_regression_binary  width='50%'}
...
![](figures/logistic_regression_multiclass){#fig:logistic_regression_multiclass}
...
```{r logistic1,cache=TRUE,dependson='mnist_onehot_final',echo=-1}
...
```{r logistic3,cache=TRUE,dependson='logistic2'}
mean(Y_test == Y_pred)
```
...
```{r logistic6,cache=TRUE,dependson='logistic5',echo=FALSE,fig.height=6}
...
![](figures/nnet){#fig:nnet width='50%'}
...
```{r deep21,cache=TRUE,dependson='mnist_onehot_final'}
model <- keras_model_sequential()
...
```{r deep23,cache=TRUE,dependson='deep22',echo=FALSE,fig.height=6}
...
```{r deep61,cache=TRUE,dependson='mnist_onehot_final'}
model <- keras_model_sequential()
...
```{r deep63,cache=TRUE,dependson='deep62',echo=FALSE,fig.height=6}
...
```{r deskew2,cache=TRUE,dependson='deskew1',echo=FALSE,fig.height=6,fig.width=12}
set.seed(123)
par(mar=c(0,0,0,0))
par(mfrow=c(10,20))
for (i in 0:9) {
...
```{r deskew3,cache=TRUE,dependson='deskew2'}
Z_train <- X_train
...
```{r deskew4,cache=TRUE,dependson='deskew3'}
model <- keras_model_sequential()
layer_dense(model, units=10, activation='softmax')
compile(model, optimizer='sgd',
        loss='categorical_crossentropy')
...
```{r deskew6,cache=TRUE,dependson='deskew5',echo=FALSE,fig.height=6}
...
```{r deep_deskew,cache=TRUE,dependson='mnist_onehot_final',echo=FALSE}
model <- keras_model_sequential()
layer_dense(model, units=800, activation='relu')
layer_dense(model, units=10,  activation='softmax')
compile(model, optimizer='sgd',
...
```{r globalsum,cache=TRUE,dependson=c('deep_deskew','deskew6','deep63','deep23','logistic6'),echo=FALSE,fig.height=5,fig.width=6}
...
```{r globalsum2,cache=TRUE,dependson=c('globalsum'),echo=FALSE,fig.height=5,fig.width=6}
par(mar=c(3,10,2,1))
