...
Vectors are 1-dimensional objects -- they represent "flat" sequences of values.
Matrices, on the other hand, are 2-dimensional -- they represent tabular data,
...
of equal lengths with each other:
...
Note that logical and character matrices are possible as well.

For instance, knowing that comparison operators such as `<` and `==`
...
`outer()` applies a given function
on each pair of elements from two vectors.
...
Also note that we can create a different *view* on the same underlaying
data vector:
