...
min_optim <- function(mu0, X)
    optim(mu0,
        function(mu, X) {
            get_loss(matrix(mu, nrow=nrow(mu0)), X)
...
print(get_loss(res_tried_very_hard, X))
```
...
to minimise a given objective/loss function $f$
...
2. Repeat until some convergence criterion is not met:
    ...
    (the smaller the loss, the greater its fitness)
...
cur_loss <- sapply(pop, get_loss, X)
cur_best_loss <- min(cur_loss)
...
Let's assume that the loss for each individual should be a function of
the rank of the objective function's value
(smallest objective/loss == highest rank/utility/fitness == best fit).

For the crossover, we will sample pairs of individuals with
probabilities inversely proportional to their losses.

```{r,dependson='genetic1',genetic2,cache=TRUE}
selection <- function(cur_loss) {
    npop <- length(cur_loss)
    probs <- rank(-cur_loss)
...
for (i in 1:niter) {
    ...
    if (cur_best_loss < best_loss) {
        best_loss <- cur_best_loss
        best_mu <- pop[[which.min(cur_loss)]]
