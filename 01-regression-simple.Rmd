...
legend("topleft",
    ...
    col=c(2,3,4),
    legend=c(
        sprintf("y=%.3fx+%03.0f; SSR=%.0f",m1$coefficients[2],m1$coefficients[1], sum(m1$residuals^2)),
        sprintf("y=%.3fx+%03.0f; SSR=%.0f",m2$coefficients[1],0, sum(m2$residuals^2)),
        sprintf("y=%.3fx+%03.0f; SSR=%.0f",m3$coefficients[2],m3$coefficients[1], sum(m3$residuals^2))
    ), bg="white")
```
...
f$coefficient[2]*X_new + f$coefficient[1]
```
...
and not the objects in the R's "workspace".
...
```{r anscombe_resid}
y_pred1 <- predict(f1, ans1) # x column in ans1...
y_pred2 <- predict(f2, ans2) # ...is only taken into account
y_pred3 <- predict(f3, ans3) # this is the same as...
y_pred4 <- predict(f4, ans4) # f4$coefficients[2]*ans4$x + f4$coefficients[1]
```
