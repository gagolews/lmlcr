...
```{r plot_rpart1,fig.height=5,echo=-1,fig.cap="A decision tree for the `wines` dataset"}
par(mar=c(0,0,0,0))
...
```{r plot_rpart222,echo=-1,fig.cap="A (simpler) decision tree for the `wines` dataset"}
par(mar=c(0,0,0,0))
...
```{r tree333,fig.height=8,echo=-1,fig.cap="A (more complex) decision tree for the `wines` dataset"}
par(mar=c(0,0,0,0))
# cp = complexity parameter, smaller → more complex tree
...
Figure {@fig:glm2} depicts the obtained probability
$\Pr(Y=1|\text{density})$.
...
{ BEGIN exercise }
Try fitting the models based on different combinations
of features.
{ END exercise }
