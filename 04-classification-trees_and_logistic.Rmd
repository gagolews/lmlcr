...
```{r plot_rpart1,fig.height=5,echo=-1}
par(mar=c(0,0,0,0))
...
```{r lm3b,echo=FALSE,cache=TRUE,dependson='lm3',cache=TRUE}
set.seed(123)
f_r <- lm(Y~density+residual.sugar+pH, data=XY_train_r)
Y_pred_r <- predict(f_r, XY_test_r)
...
```{r sigmoid,echo=FALSE,cache=TRUE}
...
```{r glm2,echo=FALSE,cache=TRUE,eval=TRUE}
