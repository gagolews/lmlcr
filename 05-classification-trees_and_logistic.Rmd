...
- In this lecture we are still  interested in  **classification** tasks;
we assume that each $y_i$ is a discrete label.
...
```{r}
# See: http://www.gagolewski.com/resources/data/
...
Note that the terms **positive** and **negative** refer the the classifier's output, i.e., occur when $\hat{y}_i$ is equal to $1$ and $0$, respectively.
...
instead of a train-validate-test split. These will be explored
in a dedicated Machine Learning unit.
...
* C4.5 algorithm  by Ross Quinlan, 1993
* C5.0, 1997
* CART by Leo Breiman et al., 1984

Decision trees are most often constructed by a *greedy*, *top-down*
*recursive partitioning*.

These should be discussed in more detail in a dedicated unit on Machine Learning.
...
points(XY_train$alcohol, jitter(as.integer(as.character(XY_train$Y)), 0.1), col=
...
(see a dedicated unit on ML) but also an intuitive interpretation.
...
> In a lecture on optimisation, we shall see that
...
- Interestingly, in the next lecture we will  note that the logistic regression model
...
- T. Hastie, R. Tibshirani, J. Friedman,
    *The Elements of Statistical Learning*, Springer, 2017
    -- Chapters 4 and 7 as well as (*) Chapters 9,  10,  13, 15
...
- T.M. Therneau, An Introduction to Recursive Partitioning Using the RPART Routines, 2019.
