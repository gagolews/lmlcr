...
An **optimisation task** deals with finding an element ${x}\in \mathbb{D}$
that minimises or maximises $f$:

\[
\min_{{x}\in \mathbb{D}} f({x}) \quad\text{or}\quad\max_{{x}\in \mathbb{D}} f({x}),
\]
...
In other words, $\min_{{x}\in \mathbb{D}} f({x})$
and $\max_{{x}\in \mathbb{D}} -f({x})$ represent the same optimisation problems
(and hence have identical solutions).

. . .

A **minimum** of $f$ is a point ${x}^*$ such that
$f({x}^*)\le f({x})$ for all ${x}\in \mathbb{D}$.

A **maximum** of $f$ is a point ${x}^*$ such that
$f({x}^*)\ge f({x})$ for all ${x}\in \mathbb{D}$.
...
$f({x}^*)\le f({x})$ for all ${x}\in \mathbb{D}$) is sometimes also referred to as
a **global** minimum.
...
```{r bfgs_multi_where,echo=FALSE,cache=TRUE,fig.cap="Each line segment connect a starting point to the point of BFGS's convergence"}
set.seed(123)
res <- replicate(1000, {
    # this will be iterated 100 times
    x12_init <- runif(2, -5, 5)
...
{ BEGIN exercise }
Food for thought:
Can we guarantee that the global minimum will be found within $N$ tries?
{ END exercise }
...
heuristics yourself. You can set `eta <- eta*0.9` in every iteration
...
Some of them are just tricks that work well in some examples
and destroy the convergence
on other ones.
