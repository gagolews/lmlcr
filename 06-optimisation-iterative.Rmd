...
Assuming that $\mathbb{D}=\mathbb{R}$, Figure {@fig:f_global_minimum}
...
Assuming that $\mathbb{D}=\mathbb{R}$, Figure {@fig:f_global_minimum_not_unique}
...
compare Figure {@fig:bfgs_multi_hist}.





```{r bfgs_multi_hist,cache=TRUE,fig.cap="A histogram of the objective function's value at the local minimum found when using a random initial guess"}
...
Figure {@fig:bfgs_multi_where} depicts all the random starting points and where
do we converge from them.

```{r bfgs_multi_where,echo=FALSE,cache=TRUE,fig.cap="Each line segment connect a starting point to the point of BFGS's convergence; note that by starting in the neighbourhood of $(0,-4)$ we can actually end up in any of the 4 local minima"}
set.seed(123)
res <- replicate(1000, {
    # this will be iterated 100 times
    x12_init <- runif(2, -5, 5)
...
Figure {@fig:gradplots} illustrates a few scaled gradients of the $g$
...
Figure {@fig:etastepplot} zooms in the contour plot so that we can see
...
see Figure {@fig:etadef2f} for the path taken.
...
And now for something completely different: $\eta=0.1$, see Figure {@fig:etadef3f}.
...
Figures {@fig:mnist_sgd} and {@fig:mnist_sgd2b} depict
...
```{r mnist_sgd,cache=TRUE,dependson='mnist_sgd1',echo=FALSE,fig.cap="Cross-entropy and accuracy on the train and test set in each iteration of SGD; batch size of 32"}
set.seed(123)
# random matrix of size 785x10 - initial guess
...
```{r cache=TRUE,mnist_sgd2b,dependson='mnist_sgd2',echo=FALSE,fig.cap="Cross-entropy and accuracy on the train and test set in each iteration of SGD; batch size of 128"}
<<mnist_sgd>>
```
...
See Figure {@fig:convex_function} for a graphical illustration of the above.


![An illustration of the definition of a convex function](figures/convex_function){#fig:convex_function width=75%}
...
all $\theta\in [0,1]$; see Figure {@fig:convex_set} for an illustration.


![A convex and a non-convex set](figures/convex_set){#fig:convex_set width=75%}
...
and $\theta\in[0,1]$, see Figure {@fig:convex_concave}.
...
![(non-)convex vs. (non-)concave functions](figures/convex_concave){#fig:convex_concave width=75%}
...
but here we are. Better *a* solution than no solution at all, remember?
