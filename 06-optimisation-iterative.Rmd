...
if defines the set of possible solution candidates.
...
> In this lecture, we will deal with **unconstrained continuous optimisation**,
i.e., we will assume the search space is $D=\mathbb{R}^p$ for some $p$.




### Example Problems in Machine Learning
...
\sum_{i=1}^n \left( \beta_0 + \beta_1 x_1^{(i)}+\dots+\beta_p x_p^{(i)} - y^{(i)} \right) ^2
\]
...
-\frac{1}{n} \sum_{i=1}^n
\left(\begin{array}{cc}
     y^{(i)} \log \left(\frac{1}{1+e^{-(\beta_0 + \beta_1 x_1^{(i)}+\dots+\beta_p x_p^{(i)})}}                                                        \right)+\\
+ (1-y^{(i)})\log \left(\frac{e^{-(\beta_0 + \beta_1 x_1^{(i)}+\dots+\beta_p x_p^{(i)}}}{1+e^{-(\beta_0 + \beta_1 x_1^{(i)}+\dots+\beta_p x_p^{(i)})}}\right)
\end{array}\right)
\]
...
Assuming that $D=\mathbb{R}$, here is an example objective function, $f:\mathbb{D}\to\mathbb{R}$,
...
y <- (x-1)^2-2
plot(x, y, type='l', xlab='x', ylab='f(x)', las=1, ylim=c(-5, 8))
abline(v=1, lty=3)
abline(h=-2, lty=3)
...
plot(x, ff(x), type='l', xlab='x', ylab='f(x)', las=1, ylim=c(-5, 8))
...
Moreover, we  cannot often be sure the result returned by an algorithm
is indeed a global minimum. Maybe there exist a better solution
...
xlab='x', ylab=expression(f[i](x)), las=1, ylim=c(-6, 8), col=c(1,2,4),lty=c(1,2,4))
legend("bottom", ncol=3, legend=c("smooth", "non-smooth", "noisy"), lty=c(1,2,4), col=c(1,2,4))
```
...
> Example: cycling from the Deakin University's Burwood Campus to the CBD
not knowing the route and with GPS disabled  -- you'll have to ask
many people along the way, but you'll eventually (because most people are good)
get to some CBD
(say, in Perth).
...
R has a built-in `optim()` function that provides an implementation
...
once you get to know about Taylor series approximation, see
the unit on Calculus/Mathematical Modelling.
...
```{r,echo=-1,warning=FALSE,fig.height=6, fig.width=6,out.width='50%',gbfgsvisit,fig.keep='none',cache=TRUE}
set.seed(123)
...
```{r,echo=-1,warning=FALSE,fig.height=6, fig.width=6,out.width='50%',echo=FALSE,cache=TRUE}
<<gbfgsvisit>>
```
...
#' @param par_generator a function generating initial guesses
#' @param ...           further arguments to optim()
...
in, e.g., the Keras manual at https://keras.rstudio.com/reference/fit.html.
Don't worry though! This will be covered in the unit dedicated to Mathematical Modelling/Calculus.
We take our time on this -- Melbourne wasn't built in a day.
...
like TensorFlow (upon which keras is built), feature
...
#' @param fn    a function to be minimised
#' @param gr    a function to return the gradient of fn
...
```{r,echo=-1,warning=FALSE,fig.height=6, fig.width=6,out.width='50%',echo=FALSE,cache=TRUE,dependson='etadef1',etastepplot}
set.seed(123)
g_vectorised_plot <- function(x12) {
    points(x12[1], x12[2], col=2, pch=3) # draw
    g(x12[1], x12[2]) # return value
}
contour(x1, x2, y, las=1, nlevels=25, xlim=c(-3, -1), ylim=c(1, 3))
...
```{r,warning=FALSE,fig.height=6, fig.width=6,out.width='50%',echo=FALSE,cache=TRUE,dependson='etadef2'}
<<etastepplot>>
```
...
```{r,warning=FALSE,fig.height=6, fig.width=6,out.width='50%',echo=FALSE,cache=TRUE,dependson='etadef3'}
<<etastepplot>>
```
...
and keep bouncing around the minimum.


\ \

This is why many optimisation libraries (including keras) implement some
of the following ideas:

* *learning rate decay* -- start with large $\eta$,
decreasing it in every operation, say, by some percent
...
Recall that in the previous lecture we have
...
\tilde{\mathbf{X}}\boldsymbol{\beta}
\right),
\]
where
$\tilde{\mathbf{X}}\in\mathbb{R}^{n\times 785}$ is a matrix
representing $n$ images of size $28\times 28$, augmented with a column of $1$s,
and
$\boldsymbol{\beta}\in\mathbb{R}^{785\times 10}$ is the coefficients matrix.

Of course, by the definition of matrix multiplication,
$\hat{\mathbf{Y}}$ will be a matrix of size
$n\times 10$, where $\hat{y}_{i,j}$ represents the predicted probability
that the $i$-th image depicts the $j$-th digit.
...
in our case $k=10$ and $p=785$):

\[
\begin{array}{rcl}
E(\boldsymbol\beta) &=& -\displaystyle\frac{1}{n} \displaystyle\sum_{i=1}^n \displaystyle\sum_{j=1}^k y_{i,j}
\log\left(
\frac{
\exp\left(
\displaystyle\sum_{u=1}^p x_{iu} \beta_{uj}
\right)
}{
\displaystyle\sum_{c=1}^k \exp\left(
\displaystyle\sum_{u=1}^p x_{iu} \beta_{uc}
\right)
}
\right)\\
&=&
...
\left(
\log \left(\displaystyle\sum_{c=1}^k \exp\left(
\displaystyle\sum_{u=1}^p x_{iu} \beta_{uc}
\right)\right)
- \displaystyle\sum_{j=1}^k y_{i,j} \displaystyle\sum_{u=1}^p x_{iu} \beta_{uj}
\right)
\end{array}
\]
...
\[
\begin{array}{rcl}
\displaystyle\frac{\partial E}{\partial \beta_{a,b}}(\boldsymbol\beta) &=&
\displaystyle\frac{1}{n} \displaystyle\sum_{i=1}^n x_{i,a}
\left(
\frac{
\exp\left(
\displaystyle\sum_{u=1}^p x_{iu} \beta_{ub}
\right)
}{
\displaystyle\sum_{c=1}^k \exp\left(
\displaystyle\sum_{u=1}^p x_{iu} \beta_{uc}
...
(with respect to the parameter matrix $\boldsymbol\beta$)
...
-- patiently pining for the Calculus/Mathematical Modelling unit --
...
\[
E(\boldsymbol\beta) =
-\frac{1}{n_\text{train}} \sum_{i=1}^n
\log \Pr(Y=y_\text{train}^{(i)}|\mathbf{x}_\text{train}^{(i)},\boldsymbol\beta)
\]
...
E(\boldsymbol\beta) =
-\frac{1}{b} \sum_{i=1}^b
\log \Pr(Y=y_\text{train}^{(\text{random\_index}_i)}|\mathbf{x}_\text{train}^{(\text{random\_index}_i)},\boldsymbol\beta)
\]
...
```{r,cache=TRUE,mnist_sgd,dependson='mnist_sgd1',echo=FALSE,fig.height=5}
set.seed(123)
# random matrix of size 785x10 - initial guess
...
matplot(stat_freq*(1:(maxit%/%stat_freq)), cbind(ce_train, ce_test), las=1, type='l', ylab="cross-entropy", xlab="iteration", log="y")
legend("top", ncol=2, legend=c("train", "test"), lty=1:2, col=1:2)
matplot(stat_freq*(1:(maxit%/%stat_freq)), cbind(acc_train, acc_test),
        las=1, type='l', ylab="accuracy", xlab="iteration",
        ylim=c(0.8, 0.93))
...
```{r,cache=TRUE,mnist_sgd2b,dependson='mnist_sgd2',echo=FALSE,fig.height=5}
<<mnist_sgd>>
```
...
You will get into their details in a dedicated unit covering
deep neural networks in more detail, but you already have developed some
good intuitions!
