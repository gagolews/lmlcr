...
The goal is to model wine quality based on physicochemical features.
...
Our aim is to build a classifier that takes 11 wine physicochemical
features and determines whether it's a "drinkable" wine.
...
* trying to find a subset of features that are relevant

That is the "more art than science" part of data science (sic!), and
hence most textbooks are not really eager for discussing such topics
(including this one).

Sorry, this is said but true. The solutions that work well in the case of dataset
...
```{r,message=FALSE}
features <- c("alcohol", "density", "pH")
Y_knn5s <- knn(Z_train[,features], Z_test[,features],
    Y_train, k=5)
mean(Y_test == Y_knn5s) # accuracy
Y_knn10s <- knn(Z_train[,features], Z_test[,features],
    Y_train, k=10)
mean(Y_test == Y_knn10s) # accuracy
```
...
wines <- read.csv("datasets/winequality-all.csv", comment="#")
X <- as.matrix(wines[,1:11])
Y <- factor(as.numeric(wines$response >= 7))
```
